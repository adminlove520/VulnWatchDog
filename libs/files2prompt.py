...
def print_path(path, content, cxml, markdown, line_numbers):
    if cxml:
        return print_as_xml(path, content, line_numbers)
    elif markdown:
        return print_as_markdown(path, content, line_numbers)
    else:
        return print_default(path, content, line_numbers)


def print_default(path, content, line_numbers):
    output = []
    output.append(path)
    output.append("---")
    if line_numbers:
...
def print_as_xml(path, content, line_numbers):
    global global_index
    output = []
    output.append(f'<document index="{global_index}">')
    output.append(f"<source>{path}</source>")
...
def print_as_markdown(path, content, line_numbers):
